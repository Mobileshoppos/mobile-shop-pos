-- This command will UPDATE the existing function to run with the permissions of the CALLER (the logged-in user).
-- This ensures that all RLS policies are correctly applied during the inventory insertion process.
...
RETURNS bigint
LANGUAGE plpgsql
SECURITY INVOKER -- This is the only line that has been added/changed.
AS $$
DECLARE
