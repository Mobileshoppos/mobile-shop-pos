-- Purane tamam functions ko unke naam se khatam karein (Overloading khatam karne ke liye)
DROP FUNCTION IF EXISTS public.record_bulk_supplier_payment(bigint, numeric, text, timestamptz, text);
DROP FUNCTION IF EXISTS public.record_bulk_supplier_payment(uuid, numeric, text, timestamptz, text);
DROP FUNCTION IF EXISTS public.edit_supplier_payment(bigint, numeric, text);

-- Ab dobara se sahi functions install karein
