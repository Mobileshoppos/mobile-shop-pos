...
BEGIN
    ...
    LOOP
        ...
        v_total_amount := v_total_amount + (v_quantity * (item->>'purchase_price')::NUMERIC);

        -- CASE A: OLD ITEM (Update + Expand)
        IF (item->>'id') IS NOT NULL THEN
            v_item_id := (item->>'id')::BIGINT;
            v_kept_item_ids := array_append(v_kept_item_ids, v_item_id); -- Asal item ko safe list mein daala

            -- 1. Asal Item ko Update karein
            UPDATE inventory
            ...
            -- 2. Agar Quantity > 1 hai, to baqi COPIES banayein
            IF v_quantity > 1 THEN
                FOR i IN 2..v_quantity LOOP
                    ...
                    SELECT product_id, variant_id, user_id, (item->>'purchase_price')::NUMERIC, (item->>'sale_price')::NUMERIC, (item->>'imei')::TEXT, (item->'item_attributes')::JSONB, p_supplier_id, p_purchase_id
                    FROM inventory WHERE id = v_item_id
                    RETURNING id INTO v_new_id;

                    v_kept_item_ids := array_append(v_kept_item_ids, v_new_id); -- Copy ko bhi safe list mein daala
                END LOOP;
            END IF;

        -- CASE B: NEW ITEM (Insert Multiple)
        ELSE
            ...
            -- Quantity ke hisaab se loop chalayein
    ...
    -- 3. Delete Logic (Ab yeh extra items ko delete kar dega agar quantity kam ki gayi ho)
