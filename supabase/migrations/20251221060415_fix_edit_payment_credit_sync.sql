...
BEGIN
    ...
    -- 4. RE-APPLY (Naya hisaab lagayein - FIFO)
    ...
    -- 5. FINAL BALANCE SYNC (YEH HAI FIX)
    -- Hum Supplier ka poora hisaab dobara check kar rahe hain taake Credit sahi ho jaye.
    ...
    -- B. Total Payments (Real Table se)
    SELECT COALESCE(SUM(amount), 0) INTO v_total_payments_real
    FROM supplier_payments WHERE supplier_id = v_supplier_id;

    -- C. Credit Calculate karein (Paid - Business)
    v_actual_credit := GREATEST(0, v_total_payments_real - v_total_business);

    -- D. Supplier Table Update karein
    UPDATE suppliers
