-- 1. Pehle purane function ko drop karte hain (taake koi purana nishaan baqi na rahe)
-- Hum ne jitne bhi purane signatures banaye thay, un sab ko drop kar dete hain safety ke liye
DROP FUNCTION IF EXISTS "public"."record_purchase_payment"(uuid, integer, integer, numeric, text, date, text);
DROP FUNCTION IF EXISTS "public"."record_purchase_payment"(uuid, uuid, uuid, numeric, text, date, text);

-- 2. Naya aur UUID compatible function banate hain
CREATE OR REPLACE FUNCTION "public"."record_purchase_payment"(
    ...
    "p_payment_date" text, -- Isay 'text' rakha hai taake JavaScript ki ISO string qubool ho jaye
    "p_notes" text
) RETURNS void
...
-- 3. Permissions dobara dete hain
GRANT ALL ON FUNCTION "public"."record_purchase_payment" TO "authenticated";
GRANT ALL ON FUNCTION "public"."record_purchase_payment" TO "service_role";