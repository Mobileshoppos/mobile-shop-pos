...
DECLARE
    ...
    -- Variables for Final Sync
    v_total_business numeric;
    v_total_payments_real numeric;
    v_total_refunds_real numeric;
    v_net_paid numeric;
...
BEGIN
    ...
    -- 5. FINAL BALANCE SYNC (UPDATED WITH REFUNDS)
    
    -- A. Total Business (Purchases)
    SELECT COALESCE(SUM(total_amount), 0) INTO v_total_business
    FROM purchases WHERE supplier_id = v_supplier_id;

    -- B. Total Payments
    SELECT COALESCE(SUM(amount), 0) INTO v_total_payments_real
    FROM supplier_payments WHERE supplier_id = v_supplier_id;

    -- C. Total Refunds (Yeh Naya Hai)
    SELECT COALESCE(SUM(amount), 0) INTO v_total_refunds_real
    FROM supplier_refunds WHERE supplier_id = v_supplier_id;

    -- D. Net Paid (Paid - Refunds)
    v_net_paid := v_total_payments_real - v_total_refunds_real;

    -- E. Credit Calculate karein
    v_actual_credit := GREATEST(0, v_net_paid - v_total_business);

    -- F. Supplier Table Update karein
    UPDATE suppliers
    SET credit_balance = v_actual_credit
