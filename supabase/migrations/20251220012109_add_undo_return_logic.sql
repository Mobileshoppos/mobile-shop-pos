-- Function: undo_return_item
-- Maqsad: Agar ghalti se item return ho gaya, to usay wapis Available karna aur hisaab theek karna.
...
BEGIN
    ...
    -- 5. Purchase ka Total aur Balance wapis barhayen
    ...
    SET total_amount = total_amount + v_price,
        balance_due = (total_amount + v_price) - v_amount_paid
    WHERE id = v_purchase_id;

    -- 6. Supplier Credit Adjust karein (Agar credit mila tha to wapis lein)
    ...
    IF (v_new_credit_contrib - v_old_credit_contrib) <> 0 THEN
        UPDATE suppliers
        SET credit_balance = credit_balance + (v_new_credit_contrib - v_old_credit_contrib)
    ...
    UPDATE purchases 
    SET status = CASE 
        WHEN ((total_amount + v_price) - v_amount_paid) <= 0 THEN 'paid' 
        ELSE 'partially_paid' 
    END 
    WHERE id = v_purchase_id;

    -- 8. Agar Return Record khali ho gaya hai (0 amount), to usay delete kar dein
