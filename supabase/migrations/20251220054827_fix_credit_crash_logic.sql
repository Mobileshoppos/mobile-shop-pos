...
BEGIN
    -- 1. Purana Data lein (Balance Due bhi mangwaya hai)
    ...
    -- 2. CREDIT REVERSAL (SMART LOGIC)
    -- Check karein ke purani overpayment kitni thi
    v_old_overpayment := COALESCE(v_old_paid, 0) - COALESCE(v_old_total, 0);
    
    IF v_old_overpayment > 0 THEN
        -- Check karein ke is overpayment mein se kitna hissa Purchase mein phansa hai (Negative Balance)
        -- Agar balance -130 hai, to matlab poora 130 purchase mein hai.
        v_credit_in_purchase := ABS(LEAST(v_old_balance_due, 0));
        
        -- Baqi jo bacha, wo Supplier ke pass hoga
        v_credit_in_supplier := v_old_overpayment - v_credit_in_purchase;

        -- Sirf utna minus karein jo Supplier ke pass tha
        IF v_credit_in_supplier > 0 THEN
            UPDATE suppliers 
            SET credit_balance = credit_balance - v_credit_in_supplier
    ...
    -- 7. CREDIT APPLICATION (New Logic)
    ...
    IF v_new_balance_raw < 0 THEN
        ...
        -- Purchase ka Balance 0 kar dein
