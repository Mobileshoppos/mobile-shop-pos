-- 1. Sales table par local_id ka UNIQUE check (Smart Check)
DO $$ 
BEGIN 
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'sales_local_id_key') THEN
        ALTER TABLE public.sales ADD CONSTRAINT sales_local_id_key UNIQUE (local_id);
    END IF;
END $$;

-- 2. Ab function ko update karein (Duplicate-Handling Logic ke sath)
